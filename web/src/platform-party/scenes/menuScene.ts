...
import { Character, MapData, PlayerMetadata } from "../models";
import { Player } from "../entities/player";
import { StageMap } from "./stage";
import StageScene from "./stageScene";
import MapBuilderScene from "./mapBuilderScene";
import { loginPlayer } from "../service";
...
export default class MenuScene extends Scene {
    ...
    public override mouseDown(event: MouseEvent) {
        if (this._startButton.isHovered) {
            ...
            this.handlePlayerLogin(this._textInput.text)
        }
    ...
    private handlePlayerLogin(playerName: string): void {
        loginPlayer(playerName, Character.BLUE).then((loginResult) => {
            // TODO: bruh
            ...
            this._fetchDefaultMap().then((stageMap: StageMap) => {
                ...
                this.manager.scene = new StageScene(this.manager, stageMap, "socket", player);
            });
        });
    }
